...
| プラン | ハンドル | 商品 ID | SKU |
|---|---|---|---|
| 初回販売 (〜11/30。新之助 5kg ＋ きたりえカレー 2 個) | `shinnosuke-5kg-kitarie-curry-set` | 7982710358067 | NJ-SHIN-5KG-CURRY |
...
- **未対応 (2026-09-26 調査済み)**: 前払式通信販売の承諾通知 (特商法 13 条) と最終確認画面の表示義務 (12 条の 6) のうち、**引渡時期は配送方法名で対応済み** (「配送」節の予約販売プロフィール。Basic プランなのでチェックアウト本体はアプリでも変えられず、フッターのポリシーリンク以外に出せる文言は商品名と配送方法名だけ)。残り: (a) 注文確認メールに承諾の一文と販売業者の住所・電話番号を足す (インボイス欄と同じ手順。既定メールの「配送方法」欄に配送方法名が出ることもプレビューで確認する。住所・電話を `shop.address` / `shop.phone` から出すなら請求先住所を会社の住所にしてから)、(b) 申込期間 (セットの 11/30 まで。期間限定販売は該当、個数限定は非該当) を最終確認画面に出す。ガイドラインが挙げる「商品名に併記」か、申込期間の定めをやめて数量限定にするかはユーザーの選択待ち、(c) チェックアウトに PayPal が有効で出ているが特商法の「お支払い方法」に無い。使わないなら管理画面「設定 > 決済」で無効化 (API では変更できない)。根拠と実機確認の記録は Obsidian `Notes/特商法 12 条の 6・13 条への Shopify Basic での対応 (najilaboule-shop, 2026-09 調査)`
