...
- ロケーションは 1 つ (ID 75946819635。名前は「新潟オフィス」(2026-09-26 にユーザーが管理画面で変更。それまでは住所から自動生成の「1-6-4」)、`fulfillsOnlineOrders: true`)。住所はストア作成時に入力したもの (会社の住所ではない可能性がある。要確認)
...
- **未決定**: 発送場所 (ロケーション「新潟オフィス」) の住所を確定して入れる。発送元が 1 か所ならロケーションは増やさず既存を書き換える。ロケーションの住所は店頭受取を使わない限りストアには表示されない
