...
- **未対応 (2026-09-26 調査済み)**: 前払式通信販売の承諾通知 (特商法 13 条) と最終確認画面の表示義務 (12 条の 6) のうち、引渡時期は配送方法名で対応済み (「配送」節の予約販売プロフィール)、申込期間はセットを数量限定にして該当しなくした (「商品」節)、PayPal は 2026-09-26 にユーザーが管理画面で無効化した (特商法の記載はそのまま)。**残りは注文確認メールの承諾欄だけ**: 承諾の一文と販売業者の名称・住所・電話番号を足す (インボイス欄と同じ手順。既定メールの「配送方法」欄に配送方法名が出ることもプレビューで確認する)。住所は `shop.address` から出せる (請求先住所は会社の住所になっている) が、電話番号は `shop.phone` が空なので、設定 > ストアの詳細に入れるかメールに直接書くかを決める。根拠と実機確認の記録は Obsidian `Notes/特商法 12 条の 6・13 条への Shopify Basic での対応 (najilaboule-shop, 2026-09 調査)`
...
- 確認済み: 管理画面のプレビュー (見本注文) で Liquid エラーなく描画される。**未確認**: 実注文で 8% 側に振り分けられること (テスト注文が必要)
