...
- **未対応 (2026-09-26 調査済み、対応方法はユーザーの選択待ち)**: (1) 前払式通信販売の承諾通知 (特商法 13 条): 注文確認メールに承諾の一文・販売業者の住所と電話番号・引渡時期が無い (受領金額・年月日・商品と数量はある)。(2) 最終確認画面の表示義務 (12 条の 6): Shopify のチェックアウト (Basic プランなのでチェックアウト本体はアプリでも変えられない) は、フッターに返金・配送・プライバシー・特商法のリンクが出るので撤回/解除はリンク方式で可。引渡時期は「配送」→ 配送ポリシー →「各商品ページに記載」とさらに転送する形で弱い。申込期間 (セットの 11/30 まで。期間限定販売は該当、個数限定は非該当) はどこにも出ない。対応案: 専用の配送プロフィール「予約販売」を作って配送方法名に発送時期を入れる (最終確認画面の配送方法欄と注文確認メールの両方に出る)、セットの商品名に期限を併記 (ガイドラインが挙げる方法)、注文確認メールに承諾・住所・電話番号の欄を足す。根拠と実機確認の記録は Obsidian `Notes/特商法 12 条の 6・13 条への Shopify Basic での対応 (najilaboule-shop, 2026-09 調査)`。**併せて要修正**: チェックアウトに PayPal が有効で出ているが、特商法の「お支払い方法」に PayPal の記載が無い
