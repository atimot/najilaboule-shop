...
- **沖縄の追加送料 1,200 円は税込** (2026-09-26 ユーザー決定。商品説明・特商法・配送ポリシーも「1,200 円（税込）」)。したがって「設定 > 税金と関税 > 追加設定 > 配送料に売上税を請求する」を ON にし、「日本」の「税の優先適用」で配送に 10% を設定する (基本税率が 8% のため。1,200 円の内税は 109 円)。どちらも API では変更できず管理画面のみ。**2026-09-26 時点: トグルを ON にしたが保存待ち** (Claude の環境では税金設定の保存が権限判定で止まるので、保存と 10% の優先適用はユーザーが行う)。反映後は `shop.taxShipping` と `draftOrderCalculate` (沖縄の住所に `shippingLine` を付ける) で送料の税額 109 円を確認する
